...
df = pd.DataFrame({'Id':jpgs, 'Prediction':pl, 'Ground_Truth': gt})
df['Prediction'] = df['Prediction'].map(dict(zip(np.arange(len(label_map)), label_map)))
df['Ground_Truth'] = df['Ground_Truth'].map(dict(zip(np.arange(len(label_map)), label_map)))
df.to_csv('submissions/valpreds_' + filestem + '.csv', index=False)